...
asyncTest("simple relationship to one", async ( assert ) => {
    ...
    class User extends Model {
        static get attributesTypes() {
            return [
                [ 'id', AttributeTypes.Integer, { min: 1 } ],
                [ 'personId', AttributeTypes.Integer, { min: 1 } ]
            ]
        }
        static get relatesTo() {
            return {
    ...
    await Promise.all([
        Person.data.bulkAdd(peoplesData),
        User.data.bulkAdd(usersData)
    ])
    ...
    for (let i = usersData.length - 1; i >= 0; i--) {
        ...
        assert.deepEqual(fetchedPerson.attributes, peoplesDataByUserId[user.id], 'user '+JSON.stringify(user)+' should fetch person '+JSON.stringify(fetchedPerson))
        const fetchedUser = await fetchedPerson.fetch('user')
        assert.deepEqual(fetchedUser.attributes, usersDataByPersonId[fetchedPerson.id], 'person '+JSON.stringify(fetchedPerson)+' should fetch user '+JSON.stringify(fetchedUser))
    }
})
...
asyncTest("simple relationship to all", async ( assert ) => {
    ...
    const peoplesData = [], dataSize = 4, gamesData = [], gamesDataByPersonId = {}
    ...
    for (let i = peoplesData.length - 1; i >= 0; i--) {
        ...
        assert.deepEqual(fetchedGames, gamesDataByPersonId[person.id], 'person '+JSON.stringify(person)+' should fetch the games '+JSON.stringify(fetchedGames))
    }
})
...
asyncTest("simple relationship to all", async ( assert ) => {
    ...
    assert.deepEqual(firstGame, undefined, 'person99 should not fetch anything, since there is no game with personId 99')
    ...
    for (let i = peoplesData.length - 1; i >= 0; i--) {
        ...
        assert.deepEqual(fetchedFirstGame.attributes, gamesDataByPersonId[person.id][0], 'person '+JSON.stringify(person)+' should fetch the game '+JSON.stringify(fetchedFirstGame.attributes))
        assert.deepEqual(fetchedLastGame.attributes, gamesDataByPersonId[person.id][3], 'person '+JSON.stringify(person)+' should fetch the game '+JSON.stringify(fetchedLastGame.attributes))
    }
})