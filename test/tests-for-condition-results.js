...
asyncTest(
    ...
    async assert => {
        ...
        assert.equal(
            await modelTestData.getInstance('string key'),
            undefined,
            "getInstance('string key') should be undefined"
        )
    }
)
...
asyncTest('where() returns a Dexie WhereClause that can be used to retrieve sorted instances', async assert => {
    ...
    assert.deepEqual(resultAsArray, [{ id: 2, name: 'Test 2' }, { id: 3, name: 'Test 3' }, { id: 4, name: 'Test 4' }])
    const resultAsInstancesArray = await modelTestData
        .where('id')
        .anyOf([2, 3])
        .or('name')
        .equals('Test 4')
