...
asyncTest(
    ...
    async assert => {
        ...
        assert.deepEqual(resultAsArray, [{ id: 1, name: 'Test 1' }, { id: 2, name: 'Test 2' }])
    }
)
...
asyncTest(
    ...
    async assert => {
        ...
        assert.equal(
            ...
            'function',
            'result should have the method "toInstancesArray"'
        )
...
asyncTest(
    ...
    async assert => {
        ...
        assert.equal(
            ...
            'function',
            'result should have the method "toMapIndexedBy"'
        )
        ...
        assert.equal(resultAsMap.size, 2, 'map returned should have two elements')
        assert.deepEqual(
            resultAsMap.get('Test 2'),
            { id: 2, name: 'Test 2' },
            "get('Test 2') should return the second record"
        )
        assert.deepEqual(
            resultAsMap.get('Test 1'),
            { id: 1, name: 'Test 1' },
            "get('Test 1') should return the first record"
        )
    }
)
...
asyncTest(
    ...
    async assert => {
        ...
        assert.equal(
            ...
            'function',
            'result should have the method "toInstancesMapIndexedBy"'
        )
        ...
        assert.equal(resultAsMap.size, 2, 'map returned should have two elements')
        assert.deepEqual(
            resultAsMap.get(1).attributes,
            { id: 1, name: 'Test 1' },
            'get(1) should return the first record'
        )
        assert.deepEqual(
            resultAsMap.get(2).attributes,
            { id: 2, name: 'Test 2' },
            'get(2) should return the second record'
        )
        resultAsMap = await modelTestData.toCollection().toInstancesMapIndexedBy('name')
        assert.ok(resultAsMap instanceof Map, 'toInstancesMapIndexedBy() returns a valid Map')
        assert.equal(resultAsMap.size, 2, 'map returned should have two elements')
        assert.deepEqual(
            resultAsMap.get('Test 2').attributes,
            { id: 2, name: 'Test 2' },
            "get('Test 2') should return the second record"
        )
        assert.deepEqual(
            resultAsMap.get('Test 1').attributes,
            { id: 1, name: 'Test 1' },
            "get('Test 1') should return the first record"
        )
    }
...
asyncTest(
    ...
    async assert => {
        ...
        assert.deepEqual(
            ...
            { id: 1, name: 'Test 1' },
            'result[0] should return the first record data'
        )
    }
)
...
asyncTest(
    ...
    async assert => {
        ...
        assert.deepEqual(
            first.attributes,
            { id: 1, name: 'Test 1' },
            'result[0] should return the first record instanciated'
        )
    }
)
...
asyncTest(
    'the Query generated returns an object with an attribute "first" with an invalid model association',
    async (assert, asyncDone) => {
        ...
        try {
            ...
            assert.equal(
                e.message,
                'This method can only be called if this object was created from a model call',
                'This method can only be called if this object was created from a model call'
            )
            asyncDone()
        }
...
asyncTest(
    ...
    async assert => {
        ...
        assert.deepEqual(
            last.attributes,
            { id: 2, name: 'Test 2' },
            'result[0] should return the last record instanciated'
        )
    }
)
