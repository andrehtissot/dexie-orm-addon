...
test('recover data as object', (assert) => {
    ...
    assert.deepEqual(
        instance.attributes,
        { id: 1, name: 'Test' },
        'the "attributes" attribute should return an object with the previous defined values'
    )
})
...
asyncTest('simple validation test', async (assert) => {
    ...
    // For record 0
    assert.equal(
        validations[0].forName.size,
        0,
        'For { record:0, validation:forName } the size of validation should be 0'
    )
    for (let validationFilter of ['forId', 'forBoth', 'forAll']) {
        assert.equal(
            validations[0][validationFilter].size,
            1,
            `For { record:0, fields:${validationFilter} } the size of validation should be 1`
        )
        assert.ok(
            validations[0][validationFilter].has('id'),
            `For { record:0, fields:${validationFilter} } has "id" in validation result`
        )
        assert.equal(
            validations[0][validationFilter].get('id'),
            'is lower than mininum',
            `For { record:0, fields:${validationFilter} } has "id" validation message = "is lower than mininum"`
        )
    }
    ...
    assert.equal(
        validations[1].forId.get('id'),
        'is not an Integer',
        'For { record:1, fields:forId } has "id" validation message = "is not an Integer"'
    )
    assert.equal(
        validations[1].forName.size,
        1,
        'For { record:1, validation:forName } the size of validation should be 1'
    )
    assert.ok(validations[1].forName.has('name'), 'For { record:1, fields:forName } has "name" in validation result')
    assert.equal(
        validations[1].forName.get('name'),
        'is not a String',
        'For { record:1, fields:forName } has "name" validation message = "is not a String"'
    )
    for (let validationFilter of ['forBoth', 'forAll']) {
        assert.equal(
            validations[1][validationFilter].size,
            2,
            `For { record:1, validation:${validationFilter} } the size of validation should be 2`
        )
        assert.ok(
            validations[1][validationFilter].has('id'),
            `For { record:1, fields:${validationFilter} } has "id" in validation result`
        )
        assert.ok(
            validations[1][validationFilter].has('name'),
            `For { record:1, fields:${validationFilter} } has "name" in validation result`
        )
        assert.equal(
            validations[1][validationFilter].get('id'),
            'is not an Integer',
            `For { record:1, fields:${validationFilter} } has "id" validation message = "is not an Integer"`
        )
        assert.equal(
            validations[1][validationFilter].get('name'),
            'is not a String',
            `For { record:1, fields:${validationFilter} } has "name" validation message = "is not a String"`
        )
    }
    ...
    for (let validationFilter of ['forName', 'forBoth', 'forAll']) {
        assert.equal(
            validations[2][validationFilter].size,
            1,
            `For { record:2, validation:${validationFilter} } the size of validation should be 1`
        )
        assert.ok(
            validations[2][validationFilter].has('name'),
            `For { record:2, fields:${validationFilter} } has "name" in validation result`
        )
        assert.equal(
            validations[2][validationFilter].get('name'),
            'is not a String',
            `For { record:2, fields:${validationFilter} } has "name" validation message = "is not a String"`
        )
    }
    ...
    for (let validationFilter of ['forId', 'forName', 'forBoth', 'forAll']) {
        assert.equal(
            validations[3][validationFilter].size,
            0,
            `For { record:3, validation:${validationFilter} } the size of validation should be 0`
        )
    }
})
...
asyncTest('saving a simple valid new record', async (assert) => {
    ...
    if (typeof db.ModelTest === 'object') {
        ...
        assert.deepEqual(
            foundInstances,
            [{ id: 1, name: 'Test' }],
            'the record retrieved should be equal to previously defined'
        )
    }
})
...
asyncTest('simple delete', async (assert) => {
    ...
    for (let expectedRecord of [user2, user1]) {
        ...
        if (typeof record.delete === 'function') {
            ...
            assert.ok(
                typeof deletePromise.then === 'function' && typeof deletePromise.catch === 'function',
...
asyncTest('delete for composite primary key', async (assert) => {
    ...
    for (var i = 8; i >= 0; i--) {
        ...
        assert.deepEqual(
            savedInstance.attributes,
            variants[i],
            `last record should be ${JSON.stringify(variants[i])} in the iteration ${i}`
        )
        assert.ok(
            await savedInstance.delete(),
            `should be able to delete record ${i} (${JSON.stringify(savedInstance.attributes)})`
        )
    }
})
