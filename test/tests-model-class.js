...
test('extend, but dont implement any method or attribute', (assert) => {
    ...
    assert.equal(
        EmptyModelTest.objectStoreName,
        'EmptyModelTest',
        'Calling EmptyModelTest.objectStoreName returns the overwritten value'
    )
    assert.throws(
        () => EmptyModelTest.attributesTypes,
        /get attributesTypes\(\) must be implemented in the extending class/,
        'get attributesTypes() must be implemented in the extending class'
    )
    assert.throws(
        () => EmptyModelTest.primaryKeys,
        /get attributesTypes\(\) must be implemented in the extending class/,
        'get attributesTypes() must be implemented in the extending class'
    )
    assert.throws(
        () => EmptyModelTest.relatesTo,
        /get relatesTo\(\) must be implemented in the extending class/,
        'get relatesTo() must be implemented in the extending class'
    )
    assert.throws(
        () => Model.attributesTypes,
        /get attributesTypes\(\) should only be called from a class that extends Model/,
        'get attributesTypes() should only be called from a class that extends Model'
    )
    assert.throws(
        () => Model.primaryKeys,
        /get primaryKeys\(\) should only be called from a class that extends Model/,
        'get primaryKeys() should only be called from a class that extends Model'
    )
    assert.throws(
        () => Model.relatesTo,
        /get relatesTo\(\) should only be called from a class that extends Model/,
        'get relatesTo() should only be called from a class that extends Model'
    )
    assert.throws(
        () => {
        ...
        /Model should never be instatiated directly/,
        'Model should never be instatiated directly'
    )
})
...
test('Calling attributesNames directly from Model', (assert) => {
    ...
    assert.throws(
        ...
        /get attributesNames\(\) should only be called from a class that extends Model/,
        'get attributesNames() should only be called from a class that extends Model'
    )
})
...
asyncTest(
    'Calling first directly from Model',
    async (assert, asyncDone) => {
        ...
        try {
            ...
            assert.equal(
                e.message,
                'data() should only be called from a class that extends Model',
                'data() should only be called from a class that extends Model'
            )
            asyncDone()
        }
...
asyncTest("geting the data should retrieve the model's table dexie object", async (assert) => {
    ...
    assert.equal(
        ModelTest.data,
        db.ObjectStoreTest,
        "Dexie's desired table object should be accessible by (Model class).data, allowing the return to be class objects instead of only stored data"
    )
})
...
asyncTest('simple find first', async (assert) => {
    ...
    assert.deepEqual(
        record.attributes,
        user1,
        'returned instance from ModelTest.data.firstInstance() has the attributes as previously defined'
    )
})
...
asyncTest('find first when objectStore is empty', async (assert) => {
    ...
    assert.equal(ModelTest.objectStoreName, 'ObjectStoreTest', 'get objectStoreName() should be overwrittable')
    assert.deepEqual(
        await ModelTest.data.firstInstance(),
        undefined,
        'returned instance should be undefined, since is a valid result when there is no record to retrieve'
    )
})
...
asyncTest('simple find first', async (assert) => {
    ...
    assert.deepEqual(
        await ModelTest.data.first(),
        user1,
        'returned object from ModelTest.data.first() has the values previously defined'
    )
})
...
asyncTest('find first when objectStore is empty', async (assert) => {
    ...
    assert.deepEqual(
        await ModelTest.data.first(),
        undefined,
        'returned object from ModelTest.data.first() has no values when there is no record to retrieve'
    )
})
...
asyncTest(
    'Calling last directly from Model',
    async (assert, asyncDone) => {
        const { Model } = newDatabase()
        try {
...
asyncTest('find last when objectStore is empty', async (assert) => {
    ...
    assert.deepEqual(
        await ModelTest.data.lastInstance(),
        undefined,
        'returned instance should be undefined, since is a valid result when there is no record to retrieve'
    )
})
...
asyncTest('simple find last', async (assert) => {
    ...
    assert.deepEqual(
        await ModelTest.data.last(),
        user2,
        'returned instance from ModelTest.data.last() has the values as previously defined'
    )
})
...
asyncTest('find last when objectStore is empty', async (assert) => {
    ...
    assert.deepEqual(
        await ModelTest.data.last(),
        undefined,
        'returned object from ModelTest.data.last() has no values when there is no record to retrieve'
    )
})
...
asyncTest(
    'Calling saveData directly from Model',
    async (assert, asyncDone) => {
        const { Model } = newDatabase()
        try {
            ...
            assert.equal(
                e.message,
                'async saveData() should only be called from a class that extends Model',
                'async saveData() should only be called from a class that extends Model'
            )
            asyncDone()
        }
...
asyncTest(
    ...
    async (assert, asyncDone) => {
        ...
        try {
            await ModelTest.saveData(records), assert.ok(false, "Model's ObjectStore doesn't exist in this database")
            asyncDone()
        } catch (e) {
            assert.equal(
                e.message,
                "Model's ObjectStore doesn't exist in this database",
                "Model's ObjectStore doesn't exist in this database"
            )
            asyncDone()
        }
...
asyncTest(
    'Calling save directly from Model',
    async (assert, asyncDone) => {
        const { Model } = newDatabase()
        try {
            ...
            assert.equal(
                e.message,
                'async save() should only be called from a class that extends Model',
                'async save() should only be called from a class that extends Model'
            )
            asyncDone()
        }
